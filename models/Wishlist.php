...
class Wishlist {
    ...
    public function getFavoriteProducts(): array {
        $sql = "SELECT wi.*, c.name as category_name FROM wishlist_items wi JOIN categories c ON wi.category_id = c.id WHERE c.name = 'Favori' AND c.type = 'wishlist' ORDER BY wi.id DESC";
        return $this->db->fetchAll($sql);
    }
}