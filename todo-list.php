...
// To-Do listesini al
$rows = $db->fetchAll("SELECT * FROM harcama_kalemleri WHERE kategori_tipi='Alınacak Ürünler' AND kategori LIKE '%todo%' AND harcama_donemi = ? ORDER BY id DESC", [$selected_month]);
                                    ...
                                    <tr style="font-size:0.85rem;">
                                        ...
                                        <td> 
                                            <span class="badge" style="background:<?= $row['tutar'] > 1000 ? '#dc3545' : ($row['tutar'] > 500 ? '#ffc107' : '#28a745'); ?>; color:#fff; font-weight:600; font-size:0.6rem; padding:0.2rem 0.4rem;">
                                                <?= $row['tutar'] > 1000 ? 'Acil' : ($row['tutar'] > 500 ? 'Önemli' : 'Normal'); ?>
                                            </span>
                                        </td>
                                        <td>
